...
if (!$url) {
  ...
  $port = $default_port;
  if (isset($_GET['port']) && array_key_exists('port', $_GET)) {
    curl_setopt($ch, CURLOPT_PORT, int($port));
  }

  if (isset($_GET['send_cookies']) && array_key_exists('send_cookies', $_GET)) {
    $cookie = array();
  ...
  curl_setopt($ch, CURLOPT_USERAGENT, (isset($_GET['user_agent']) && array_key_exists('user_agent', $_GET)) ? $_GET['user_agent'] : $_SERVER['HTTP_USER_AGENT']);
...
if (isset($_GET['mode']) && array_key_exists('mode', $_GET) && $_GET['mode'] == 'native') {
  if (!$enable_native) {
  ...
  if (isset($_GET['full_headers']) && array_key_exists('full_headers', $_GET)) {
    $data['headers'] = array();
  ...
  if (isset($_GET['full_status']) && array_key_exists('full_status', $_GET)) {
    $data['status'] = $status;
  } else {
    $data['status'] = array();
